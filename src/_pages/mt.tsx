...
export function GroupChallenge() {
  const { user_id } = useSurveyStore(); // ✅ Aqui é permitido
  const navigate = useNavigate();
  const { flux } = useParams();
  const { selection } = useSelection();

  const effectiveSelection = selection || JSON.parse(localStorage.getItem("selection") || "null");
...
const allCorrect = questions.every((q) => Object.values(activeHits).includes(q.product));
    ...
    <BlankPage>
      <div className="relative flex flex-1 flex-col w-full h-full items-center justify-center gap-16">
        ...
        <div className="relative w-[1030px] aspect-[9/16] rounded-4xl overflow-hidden shadow-2xl mt-[120px]">
          ...
          {/* Hexágonos com perguntas ou imagens */}
          {questions.length > 0 && (
